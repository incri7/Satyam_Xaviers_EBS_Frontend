...
export const EditClassModal: React.FC<EditClassModalProps> = ({ isOpen, onClose, classData }) => {
    ...
    const mutation = useMutation({
        mutationFn: (data: { id: number; name: string }) => academicsService.updateClass(data.id, { name: data.name }),
        onSuccess: () => {
        ...
        onError: (err: any) => {
            setError(err.response?.data?.detail || err.message || 'Failed to update class');
        }
    });
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        mutation.mutate({ id: classData.id, name });
    };
